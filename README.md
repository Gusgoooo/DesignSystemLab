...
Design System Lab 的流程不是从零发明一套审美词典，而是把优质 AI design skill 的资产拆成可路由、可检索、可验证的知识层。外部资产通过 raw GitHub URL 分散引用，入口记录在：
...
本地规则、`PRODUCT.md`、`DESIGN.md`、token contract 和项目业务逻辑永远优先。外部资产只提供语言、数据、生成策略和质检参照，不直接覆盖本项目的 UI。
...
Local Product Context
  -> Page Type Workflow
  -> Local Design Rules
  -> External Skill Assets when requested
  -> Seed Token
...
- 改 dashboard：先读本地 page-type workflow、page-shell、dashboard、metrics/table/filter 规则；需要外部判断时，再读 Impeccable product/layout/audit 和 UIUXPROMAX products/charts/style rows。
